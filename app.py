...
# ├─ requirements.txt
# ├─ .streamlit/
# │  └─ secrets.toml       # 로컬 실행 시 선택사항, Git 업로드 금지
# └─ data/
#    ├─ seoul.shp
#    ├─ seoul.shx
#    ├─ seoul.dbf
...
#
# 배포 환경에서는 절대경로를 사용하지 않는다.
# 1) LOGI_DATA_DIR 환경변수가 있으면 해당 경로 사용
# 2) app.py 옆에 data/ 폴더가 있으면 data/ 사용
# 3) 아니면 app.py와 같은 폴더에서 데이터 파일 탐색
# ============================================================

APP_DIR = Path(__file__).resolve().parent

_env_data_dir = os.getenv("LOGI_DATA_DIR", "").strip()
...
def check_required_files():
    """배포 환경에서 필요한 데이터 파일 존재 여부를 먼저 확인한다."""
    required = [
        ROAD_SHP_PATH,
        ROAD_SHP_PATH.with_suffix(".shx"),
        ROAD_SHP_PATH.with_suffix(".dbf"),
        DEPOT_PATH,
        DRIVERS_PATH,
        DELIVERIES_PATH,
        KNOWLEDGE_PATH,
    ...
    if missing:
        st.error("필수 데이터 파일을 찾을 수 없습니다.")
        st.caption(
            "app.py 옆의 data 폴더에 파일을 넣거나 "
            "LOGI_DATA_DIR 환경변수로 데이터 폴더를 지정하세요."
        )
        st.code(
            "\n".join(
                str(p.relative_to(APP_DIR))
                if p.is_relative_to(APP_DIR)
                else str(p)
                for p in missing
            )
        )
